...
age_range_by_week <- age_range %>% 
  group_by(week) %>% 
  summarize(sum(enough_of_the_kinds_of_food_wanted) / (sum(enough_of_the_kinds_of_food_wanted) + sum(enough_food_but_not_always_the_kinds_wanted) + sum(sometimes_not_enough_to_eat) + sum(often_not_enough_to_eat) + sum(did_not_report) )) %>% 
...
#sum both of the different kinds of "enough food"?
#and divide by total responses
