...
plot_data <- covid_food_no_us %>%
  ...
  mutate(response_ratio = responses / sum(responses)) %>%
  filter(freq_feel_anxious == 'Not at all', week %in% c('week1', 'week2'))

us_merged <- merge(plot_data, us_states, by = 'location')
  
  

ggplot(data = us_merged, aes(geometry = geometry)) +
  geom_sf(aes(fill = response_ratio), color = "white", size = 0.1) +
  theme_void() +
